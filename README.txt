Please get Python 3 (please do it though anaconda) and get the packages listed at the bottom. Then you should be able to run any of the files called the classifier names. It would graph and show you the grid search results. To run a dataset, please comment the other dataset. Also, some classifers cannot run when the y axis is one hot encoded, so if those warning arises, please find the one hot encoded section of the code and disable it. (In help_methods.py and marked out for you.) Also for SVMs, there is a need to run grid search with x label being logorithmic, please enable it. (Also in helper_methods.py)
